...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="businessuser/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
						...
						<c:forEach items="${nameList}" var="var" varStatus="vs">
							<tr>
								...
								<td style="width: 260px;vertical-align: middle;" class="center" >
										...
										<span class="label label-large label-grey arrowed-in-right arrowed-in">请查看申请商家是否处理</span>
										</c:if>
										<c:if test="${var.varList.SQFLAG==2}">
										<c:if test="${QX.edit == 1}">
										...
										<c:if test="${var.varList.FLAG==1}">
											<a class="btn btn-small btn-danger" onclick="gFlag('${var.varList.BUSINESSUSER_ID}','3');" >下架</a>
											<a class="btn btn-small btn-danger" onclick="gFlag('${var.varList.BUSINESSUSER_ID}','2');" >禁用</a>
										</c:if>
										<c:if test="${var.varList.FLAG==2}">
											<a class="btn btn-small btn-danger" onclick="gFlag('${var.varList.BUSINESSUSER_ID}','3');" >下架</a>
											<a class="btn btn-small btn-success" onclick="gFlag('${var.varList.BUSINESSUSER_ID}',1);" >激活</a>
										</c:if>
										<c:if test="${var.varList.FLAG==3}">
											<a class="btn btn-small btn-success" onclick="gFlag('${var.varList.BUSINESSUSER_ID}','1');" >上架</a>
										</c:if>
										</c:if>
										<c:if test="${QX.del == 1 }">
											<a class="btn btn-small btn-danger" onclick="del('${var.varList.BUSINESSUSER_ID}');" >删除</a>
										</c:if>
										</c:if>										<!--<div class="inline position-relative">
										<button class="btn btn-mini btn-info" data-toggle="dropdown"><i class="icon-cog icon-only"></i></button>
										<ul class="dropdown-menu dropdown-icon-only dropdown-light pull-right dropdown-caret dropdown-close">
											<c:if test="${QX.edit == 1 }">
											<li><a style="cursor:pointer;" title="编辑" onclick="edit('${var.varList.BUSINESSUSER_ID}');" class="tooltip-success" data-rel="tooltip" title="" data-placement="left"><span class="green"><i class="icon-edit"></i></span></a></li>
											</c:if>
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${var.varList.BUSINESSUSER_ID}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</ul>
										</div>
									-->
