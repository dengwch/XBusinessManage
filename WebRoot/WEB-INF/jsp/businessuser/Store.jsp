...
<input type="hidden" name="BUSINESSUSER_ID" id="BUSINESSUSER_ID" value="${pd.BUSINESSUSER_ID}"/>
   ...
   <div class="container" style="padding-bottom: 100px;">
		<div class="row clearfix">
			...
			<p>主营业务：已选:${nameString}<br/>
				<!--<select name="WORK" class="form-control">
					--><c:forEach items="${workList}" var="var" varStatus="vs">
						<!--<option value="${var.WORK_ID}" <c:if test="${var.WORK_ID==pd.WORK }">selected</c:if>>${var.WORKNAME}</option>
					-->
					<input type='checkbox' name='WORK' value="${var.WORK_ID}" /> ${var.WORKNAME}
					
					</c:forEach>
			    <!--</select>
			--></p>
			<p>联系电话：<input style="width:300px;" name="TEL" id="TEL" value="${pd.TEL}"></p>
			<p>店铺地址：</p>
			<p><textarea style="margin-left:0px;" name="SHOPADDRESS" id="SHOPADDRESS">${pd.SHOPADDRESS}</textarea></p>
			<p style="margin-left: -15px;">店铺LOGO：
				<div id="addCommodityIndex">  
			        ...
			        <img border="0" id="shareImg1" src="${pageContext.request.contextPath }/static/images/photo_icon.png" width="90" height="90" onclick="$('#previewImg').click();"/>
			        </c:if>
			        <c:if test="${pd.SHOPLOGO!='' && pd.SHOPLOGO!=null}">
			        
			         	<c:if test="${fn:substring(pd.SHOPLOGO, 0, 3)=='gzh'}">
				        	<img border="0"  id="shareImg1" src="<%=OSSUrl1 %>${pd.SHOPLOGO}" width="90" height="90" onclick="$('#previewImg').click();"/>
				        </c:if>
				        <c:if test="${fn:substring(pd.SHOPLOGO, 0, 3)!='gzh'}">
				        	<img border="0"  id="shareImg1" src="<%=OSSUrl %>${pd.SHOPLOGO}" width="90" height="90" onclick="$('#previewImg').click();"/>
				        </c:if>
			        
			        </c:if>
		                     </div>         
		                    <input id="previewImg" name="myfile" type="file" onchange="uploadphoto1('previewImg')" style="display:none;"/> 
		            </div>
		            <!--input-group end-->    
				</div>
			</p>
			<p><span style="margin-left:10px;">(请尽量上传正方形的logo更美观一些)</span></p>
