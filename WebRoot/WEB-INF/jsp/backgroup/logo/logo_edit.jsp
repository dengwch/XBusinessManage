...
function checkSize(id){
	...
	function save(){
		...
		if(checkTupian($("#LOGOIMGURL").val(),"logo图片")){
			return false;
		}
		if($("#LOGOIMGURL").val()!=""){
			if(checkSize("LOGOIMGURL")){
				return false;
			}
		}
		$("#Form").submit();
...
<body>
	<form action="logo/${msg }.do" name="Form" id="Form" method="post" enctype="multipart/form-data">
		<input type="hidden" name="LOGO_ID" id="LOGO_ID" value="${pd.LOGO_ID}"/>
		<div id="zhongxin">
		<table id="table_report" class="table table-striped table-bordered table-hover">
			<tr>
				<td style="width:70px;text-align: right;padding-top: 13px;">logo图片:</td>
				<td>
				<input type="file" name="LOGOIMGURL" id="LOGOIMGURL" />
				<c:if test="${pd.LOGOIMGURL!=null&&pd.LOGOIMGURL!='' }">
				<img src="<%=OSSUrl %>${pd.LOGOIMGURL }" style="width:200px;">
					
				</c:if>
<!-- 				<input type="text" name="LOGOIMGURL" id="LOGOIMGURL" value="${pd.LOGOIMGURL}" maxlength="32" placeholder="这里输入logo图片" title="logo图片"/> -->
				</td>
			</tr>
			<tr>
